...
func IsXAccessPolicyAccepted(p *v1alpha1.XAccessPolicy) bool {
	if len(p.Status.Ancestors) == 0 {
		return false
	}
	for _, ancestor := range p.Status.Ancestors {
		if !meta.IsStatusConditionTrue(ancestor.Conditions, string(v1alpha1.PolicyConditionAccepted)) {
			return false
		}
	}
	return true
...
func IsXAccessPolicyProgrammed(p *v1alpha1.XAccessPolicy) bool {
	if len(p.Status.Ancestors) == 0 {
		return false
	}
	for _, ancestor := range p.Status.Ancestors {
		if !meta.IsStatusConditionTrue(ancestor.Conditions, string(v1alpha1.PolicyConditionProgrammed)) {
			return false
		}
	}
	return true
