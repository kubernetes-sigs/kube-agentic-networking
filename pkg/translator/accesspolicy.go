...
func (t *Translator) buildGatewayLevelRBACFilters(gateway *gatewayv1.Gateway, apTranslation *accessPolicyTranslationCollector) ([]*hcm.HttpFilter, error) {
	...
	for i, policy := range gwPolicies {
		...
		rbacProto := t.buildRBACConfigWithCommonPolicies(policy, apTranslation)
...
func (t *Translator) buildBackendLevelRBACOverrides(backend *agenticv0alpha0.XBackend, apTranslation *accessPolicyTranslationCollector) (map[string]*anypb.Any, error) {
	...
	for i, policy := range backendPolicies {
		...
		rbacConfig := t.buildRBACConfigWithCommonPolicies(policy, apTranslation)
...
func (t *Translator) buildRBACConfigWithCommonPolicies(accessPolicy *agenticv0alpha0.XAccessPolicy, apTranslation *accessPolicyTranslationCollector) *rbacv3.RBAC {
	rbacConfig := t.translateAccessPolicyToRBAC(accessPolicy, apTranslation)
...
func (t *Translator) translateAccessPolicyToRBAC(accessPolicy *agenticv0alpha0.XAccessPolicy, apTranslation *accessPolicyTranslationCollector) *rbacv3.RBAC {
			...
			case agenticv0alpha0.AuthorizationRuleTypeExternalAuth:
				if rule.Authorization.ExternalAuth != nil {
					...
					if err != nil {
						...
						apTranslation.record(accessPolicy.Namespace, accessPolicy.Name, fmt.Sprintf("rule %q: cannot fingerprint external authorization config: %v", rule.Name, err))
						continue
					}
