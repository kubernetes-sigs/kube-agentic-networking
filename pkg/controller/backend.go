...
func (c *Controller) enqueueBackend(backend *agenticv0alpha0.XBackend) {
	key := backend.Namespace + "/" + backend.Name
	c.backendqueue.Add(key)
}

// syncBackend manages the XBackend finalizer
func (c *Controller) syncBackend(ctx context.Context, key string) error {
