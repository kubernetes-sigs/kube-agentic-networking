...
func TestIsPolicyUnderTargetLimit(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			existing: []runtime.Object{
				&agenticv0alpha0.XAccessPolicy{
					ObjectMeta: metav1.ObjectMeta{Name: "policy-new", Namespace: ns, CreationTimestamp: later},
					Spec: agenticv0alpha0.AccessPolicySpec{
						TargetRefs: []gwapiv1.LocalPolicyTargetReferenceWithSectionName{{
							LocalPolicyTargetReference: gwapiv1.LocalPolicyTargetReference{
								Group: gwapiv1.Group(agenticv0alpha0.GroupName),
		...
		},
		{
			...
			existing: []runtime.Object{
				&agenticv0alpha0.XAccessPolicy{
					ObjectMeta: metav1.ObjectMeta{Name: "policy-existing", Namespace: ns, CreationTimestamp: earlier},
					Spec: agenticv0alpha0.AccessPolicySpec{
						TargetRefs: []gwapiv1.LocalPolicyTargetReferenceWithSectionName{{
							LocalPolicyTargetReference: gwapiv1.LocalPolicyTargetReference{
								Group: gwapiv1.Group(agenticv0alpha0.GroupName),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allPolicies := append(tt.existing, tt.currentPolicy)
			...
			c := &Controller{
				...
				maxAccessPoliciesPerTarget: tt.limit,
			}
