...
func TestDedupeStringsPreserveOrder(t *testing.T) {
	tests := []struct {
		...
		{name: "skips empty strings", in: []string{"a", "", "b", ""}, want: []string{"a", "b"}},
		{name: "dedupes", in: []string{"x", "y", "x", "z", "y"}, want: []string{"x", "y", "z"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := dedupeStringsPreserveOrder(tt.in)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("dedupeStringsPreserveOrder() = %#v, want %#v", got, tt.want)
			}
		})
	}
}
...
func TestReconcileAccessPolicyTranslationStatus_setsInvalid(t *testing.T) {
	...
	fakeAgentic := agenticclient.NewSimpleClientset(policy)
