...
type Controller struct {
	...
	backendqueue workqueue.TypedRateLimitingInterface[string]
...
func New(
	...
	c := &Controller{
		...
		backendqueue: workqueue.NewTypedRateLimitingQueueWithConfig(
			workqueue.DefaultTypedControllerRateLimiter[string](),
			workqueue.TypedRateLimitingQueueConfig[string]{Name: "backend"},
		),
...
func (c *Controller) Run(ctx context.Context, workers int) error {
	...
	klog.InfoS("Starting backend workers (finalizers)", "count", workers)
	for i := 0; i < workers; i++ {
		go wait.UntilWithContext(ctx, c.runBackendWorker, time.Second)
	}
...
func (c *Controller) runBackendWorker(ctx context.Context) {
	for c.processNextBackendItem(ctx) {
	}
}

func (c *Controller) processNextBackendItem(ctx context.Context) bool {
	obj, shutdown := c.backendqueue.Get()
	if shutdown {
		return false
	}
	defer c.backendqueue.Done(obj)
	if err := c.syncBackend(ctx, obj); err != nil {
		c.backendqueue.AddRateLimited(obj)
		klog.ErrorS(err, "Error syncing backend", "key", obj)
		return true
	}
	c.backendqueue.Forget(obj)
	return true
}
