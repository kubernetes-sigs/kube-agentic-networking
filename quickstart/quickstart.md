...
### Step 5.2: Configure the Envoy Sidecar

The agent uses an Envoy sidecar to establish mTLS-secured connections to the Gateway. Deploy the sidecar configuration into the `quickstart-ns` namespace:

```shell
kubectl apply -f quickstart/adk-agent/sidecar/sidecar-configs.yaml
```

### Step 5.3: Deploy the Agent
...
> **Note**: The agent connects to the proxy via the Envoy sidecar listening on `127.0.0.1:10001`. The sidecar then forwards traffic to the Gateway proxy (e.g., `envoy-proxy-<hash>.quickstart-ns.svc.cluster.local:10001`) using mTLS. 
> 
> The proxy service name is generated by hashing the Gateway's namespace and name (e.g., `envoy-proxy-<hash>`). If these change, the address here must be updated to match the new service name.
