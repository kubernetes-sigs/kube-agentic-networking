...
The `v0.2.0` release introduces Layer 7 (L7) parameter matching for Model Context Protocol (MCP) tool calls in `XAccessPolicy`, integrates OpenTelemetry-based gateway authorization tracing for agent tool calls, and enhances route status reporting.
Key highlights in this release:

- **L7 Parameter Matching**: Extended `XAccessPolicy` to support Layer 7 matching rules on MCP tool call parameters.
- **Observability Integration**: Added gateway-level authorization tracing spans for agent tool invocations in the reference implementation.
- **Routing & Infrastructure Improvements**: Updated `HTTPRoute` status condition reporting when LoadBalancers are pending, and added quickstart smoke tests to the E2E suite.

## Changes by Kind

### Feature

- Add additional request matchers to XAccessPolicy (#309, @LiorLieberman)
- Support l7 params (#315, @LiorLieberman)
- Adds observability stack integration to the reference implementation (#280, @evaline-ju)

### Bug or Regression

- Set HTTPRoute status conditions when LoadBalancer is pending (#323, @chuangw6)
- Fix BASE_REF slash sanitization for Docker image tags (#306, @david-martin)

### Documentation

- Update observability proposal evaluation to decision spans (#312, @evaline-ju)
- Document MCP params matcher context for XAccessPolicy (#291, @LiorLieberman)
- Add conformance overview documentation (#316, #318, @LiorLieberman)
- Align website and docs (#325, @LiorLieberman)
